...
- (id) initWithFrame:(CGRect)frame {
    if ( (self = [super initWithFrame:frame] )) {
        ...
        for (int i =0; i < frame.size.width/2; i++) {
...
-(void)removeAllPoints{
    [averagePointArray removeAllObjects];
    [peakPointArray removeAllObjects];
    for (int i =0; i < self.frame.size.width/2; i++) {
        [averagePointArray addObject:[NSNumber numberWithFloat:0.0f]];
        [peakPointArray addObject:[NSNumber numberWithFloat:0.0f]];
    }
}
...
{       
    ...
    {
        CGPoint point = CGPointMake(i*2, self.bounds.size.height-([[averagePointArray objectAtIndex:i] floatValue]*self.bounds.size.height));
        //增加一个点 进行连线
...
//    CGContextClearRect(context, self.bounds);
    ...
    for (int i = 1; i < [peakPointArray count]; i++)
    {
        CGPoint point = CGPointMake(i*2, ([[averagePointArray objectAtIndex:i] floatValue]*self.bounds.size.height));
        CGContextAddLineToPoint(context, point.x, point.y);
    } 
    CGContextStrokePath(context); 
}
