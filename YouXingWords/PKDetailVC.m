...
@interface PKDetailVC()<UICollectionViewDelegate,UICollectionViewDataSource,UITextFieldDelegate,YXAnswerWrong,YXTimeProgressDelegate,RestartOrNextDelegate>{
    NSTimer *timer;
...
@property (nonatomic,copy) NSMutableArray   *answerResultArr;
...
-(NSMutableArray *)answerResultArr{
    if (!_answerResultArr) {
        _answerResultArr = [NSMutableArray array];
    }
    return _answerResultArr;
}
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (_knownStr.length >=_OriginalStr.length) {
        ...
        NSDictionary *anwserDic=@{@"qId":model.qID,@"answer":_knownStr};
        [self.answerResultArr addObject:anwserDic];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        //选择题结束
        QsModel *model  =[self.list objectAtIndex:_currentQs-1];
        NSDictionary *anwserDic=@{@"qId":model.qID,@"answer":[self.answerArray[indexPath.row] option]};
        [self.answerResultArr addObject:anwserDic];
        ...
        if ([_rightAnswer isEqualToString:[self.answerArray[indexPath.row] option]]) {
            ...
            if (model.qAudio) {
                ...
                AudioServicesAddSystemSoundCompletion(_soundID, NULL, NULL, pkCompletionCallback, (__bridge void *)self);
...
void pkCompletionCallback(SystemSoundID soundID,void *clientData){
    //[SVProgressHUD showInfoWithStatus:@"请跟读"];
    //    AudioServicesDisposeSystemSoundID(soundID);
    if (((__bridge PKDetailVC *)clientData).shouldGoNext) {
        ((__bridge PKDetailVC *)clientData).shouldGoNext=NO;
        [((__bridge PKDetailVC *)clientData) getNextQs];
    }
    if (((__bridge PKDetailVC *)clientData).wrongGoNext) {
        ((__bridge PKDetailVC *)clientData).wrongGoNext=NO;
        [(__bridge PKDetailVC *)clientData afterReadAnswerWrong];
    }
...
//        }
        ...
        [self.rightArr writeToFile:rightPath atomically:NO];
        [self.wrongArr writeToFile:wrongPath atomically:NO];
        
        NSString *jsonStr=[self.answerResultArr mj_JSONString];
        NSDictionary *paraDic=@{@"token":[Utils getCurrentToken],@"contestId":self.contestId,@"submitStatus":@"1",@"answers":jsonStr,@"startTime":[NSString stringWithFormat:@"%ld",(NSInteger)_startTimeStamp],@"submitTime":[NSString stringWithFormat:@"%ld",(NSInteger)[Utils getCurrentTimeStamp]]};
        
        [NetWorkingUtils postWithUrl:SubmitContestInfo params:paraDic successResult:^(id response) {
            if ([[response valueForKey:@"status"]isEqualToNumber:@1]) {
                ...
                contestReport.correctNum=_rightNum;
                contestReport.wrongtNum=_wrongNum;
                contestReport.useTime=([Utils getCurrentTimeStamp] - _startTimeStamp)/1000;
                contestReport.contestId=self.contestId;
    ...
    if (model.qAudio) {
        ...
        NSString *finalPath=[NSString stringWithFormat:@"%@/%@/%@",filePathBase,[AnswerTools getPKID],@"txt/wordInfos.json"];
...
#pragma mark 改动
        //start
#pragma mark xl_时间条隐藏了  7.29-17：56
...
-(void)answerRight{
    ...
    QsModel *model  =[self.list objectAtIndex:_currentQs-1];
    NSDictionary *dic = @{@"word":model.qWord,@"explain":model.qExplain};
    [self.rightArr addObject:dic];
...
-(void)naviPop:(UIButton *)tap{
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"退出PK？" message:@"返回将丢失本次PK数据" preferredStyle:UIAlertControllerStyleAlert];
...
-(void)iDontKnowOnClick:(UIButton *)button{
    for (int i=0; i<self.answerArray.count; i++) {
        if ([_rightAnswer isEqualToString:[self.answerArray[i] option]]) {
            ChoiceFatherCell *cell = (ChoiceFatherCell *)[_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:i inSection:0]];
            [cell didSelected];
        }
    }
...
-(void)beginTimer{
    ...
    _timerCount--;
    _needReview.text=[self timeChange:_timerCount];

    
}
-(void)viewWillAppear:(BOOL)animated{
    //记录开始答题时间
    _startTimeStamp=[Utils getCurrentTimeStamp];
...
#pragma mark---获取当前时间与结束时间的时间差
-(NSTimeInterval)getCurrentTimeToStopTime:(id)timeStamp
{
...
-(void)setTimeStr:(NSString *)timeStr{
        _timerCount=[self getCurrentTimeToStopTime:timeStr];
