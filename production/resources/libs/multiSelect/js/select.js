...
(function (a) {
    a.fn.render = function (data) {
        if (typeof data !== undefined) {
            a(this).data("data", data);
        }
        if (a(this).is("select")) {
...
jQuery.SelectBox = function (selectobj) {
    ...
    var buttonEnabledClass = $select.attr('button-enabled-class') || 'selBtn';
    var buttonDisabledClass = $select.attr('button-disabled-class') || 'selBtn_disabled';
    ...
    var $selBtn;
    $selBtn = $("<input type='button' value=' ' class='" + buttonEnabledClass + "'/>");
    ...
    if ($select.attr("selWidth") != null) {
        inputWidth = Number($select.attr("selWidth")) - 68
    }
    ...
    if ($select.attr("disabled") == "disabled" || $select.attr("disabled") == "true" || $select.attr("disabled") == true) {
        ...
        $selBtn.addClass(buttonDisabledClass);
    ...
    function setHeight() {
        ...
        if (colNum == 1) {
            ...
            if ($lis % colNum == 0) {
                oldHeight = $lis * 40 / colNum
            } else {
                oldHeight = ($lis - $lis % colNum) * 40 / colNum + 40
            }
        }
        ...
        if (colNum != 1) {
            $container.width((colWidth + 10) * colNum + 10)
        } else {
            if (!$select.attr("boxWidth")) {
        ...
        if (boxHeight != 0) {
            ...
            if ($select.attr("openDirection") == "top") {
                ...
                } else if ($select.attr("openDirection") == "right") {
        ...
        if (!$select.attr("boxWidth")) {
            if ($container.width() < inputWidth + 38) {
                $container.width(inputWidth + 38)
            }
        }
    }
    ...
    function createOptions(data, parentid, colNum, colWidth, isEditable, rel, ul, dataRoot, paramsObj) {
        ...
        if ($select.attr("prompt") != null) {
            ...
            if (selectedIdx == -1 && selectedValue == "") {
                ...
                $input.val(li0.promptText);
            }
            ul.appendChild(li0);
            $select[0].options.length = 0;
            // $select[0].options[$select[0].options.length] = new Option('清空', '');
            if (colNum != 1) {
